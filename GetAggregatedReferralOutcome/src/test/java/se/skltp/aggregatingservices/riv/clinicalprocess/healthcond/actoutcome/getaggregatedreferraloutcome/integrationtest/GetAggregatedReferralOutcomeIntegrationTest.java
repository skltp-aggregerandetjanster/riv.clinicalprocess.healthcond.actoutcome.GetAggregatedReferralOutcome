...
public class GetAggregatedReferralOutcomeIntegrationTest extends AbstractAggregateIntegrationTest {
	...
	private static final String DEFAULT_SERVICE_ADDRESS = GetAggregatedReferralOutcomeMuleServer.getAddress("SERVICE_INBOUND_URL");

	protected String getConfigResources() {
		return
			"soitoolkit-mule-jms-connector-activemq-embedded.xml," +
	  		"GetAggregatedReferralOutcome-common.xml," +
//          Only load GetAggregatedReferralOutcome-common.xml, it will import the other config files since mule-deploy.properties can't load config-files from jar-files on the classpath, e.g. agp-core.jar
...
//	        "aggregating-service.xml," +
	...
	private List<ProcessingStatusRecordType> doTest(String registeredResidentId, String senderId, String originalConsumerHsaId, int expectedProcessingStatusSize, ExpectedTestData... testData) {
		...
		GetAggregatedReferralOutcomeTestConsumer consumer = new GetAggregatedReferralOutcomeTestConsumer(DEFAULT_SERVICE_ADDRESS, senderId ,originalConsumerHsaId);
